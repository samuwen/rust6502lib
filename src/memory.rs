...
impl Memory {
  ...
  /// Then arranges the values in little endian and returns the value at the address result.
  pub fn get_pre_indexed_data(&self, operand: u8, register: u8) -> u8 {
    let mut lsb = self.get_lsb(operand + register);
    let msb = self.get_msb(operand + register);
    lsb.push_str(&msb);
    let index = u16::from_str_radix(&lsb, 16).unwrap();
    self.mem[index as usize]
  }
  ...
  /// to the address, then returns the value at the address result.
